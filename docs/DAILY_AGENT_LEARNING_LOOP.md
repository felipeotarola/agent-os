...
- Run `npm run check:cron-review-preflight -- --optional=<path>` when optional context or publish state is ambiguous. Its result makes unavailable evidence explicit, skips push on a synced branch, and preserves verified local work after a failed push.
